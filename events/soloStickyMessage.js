...
const MIN_REPOST_INTERVAL_MS = 5 * 60 * 1000; // NEW: Only repost if last post was 5 minutes ago
...
async function repostStickyMessage(client, db, stickyMessageData) {
    ...
    // const mobChannelOriginalName = stickyMessageData.mobChannelOriginalName; // Not strictly needed here
    ...
    const now = Date.now();
    const lastPostedTimestamp = stickyMessageData.lastPostedTimestamp || 0;
    
    // --- NEW RATE LIMIT CHECK ---
    if (now - lastPostedTimestamp < MIN_REPOST_INTERVAL_MS) {
        // console.log(`Sticky Message Manager: Repost skipped for #${channel.name} due to rate limit.`);
        return;
    }
    ...
    try {
        ...
        await updateDoc(stickyDocRef, {
            ...
            lastPostedTimestamp: now, // Use current time for new timestamp
            expirationTimestamp: now + STICKY_DURATION_MS // Reset expiration on repost
        });
