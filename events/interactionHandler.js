...
module.exports = {
    ...
    async execute(interaction, db, client, isFirestoreReady, APP_ID_FOR_FIRESTORE) {
        ...
        if (interaction.customId.startsWith('damage_calc_')) {
            await this.handleDamageCalcInteraction(interaction, db, client, APP_ID_FOR_FIRESTORE);
            return;
        }
        ...
        } else if (interaction.isButton() && (interaction.customId.startsWith('page_prev_') || interaction.customId.startsWith('page_next_'))) {
            await interaction.deferUpdate();
        ...
        } else if (interaction.isButton() && interaction.customId.startsWith(TRIVIA_EXPLANATION_BUTTON)) {
             await interaction.deferUpdate();
            const parts = interaction.customId.split('_');
    ...
    handleDamageCalcInteraction: async (interaction, db, client, APP_ID_FOR_FIRESTORE) => {
