...
client.on('interactionCreate', async interaction => {
    ...
    // New, more robust interaction handling.
    ...
    // Then, check for button interactions and handle them directly
    else if (interaction.isButton()) {
        if (interaction.customId.startsWith('toggle_')) {
            await handleNotifyButton(interaction, db);
        }
    } 
    // And finally, check for select menus.
...
// A new function in index.js to handle notify buttons specifically.
async function handleNotifyButton(interaction, db) {
    ...
    try {
        ...
        // Get updated preferences to re-render the message
        const currentPrefs = {};
        const prefsRefs = {
        ...
        // Recreate the message components
